...
class WindFieldSolver:
    ...
    #######################################################################
    def __init__(self, lat_grid, lon_grid, center_lat, center_lon, vort, div, radius=333, dx=111e3/4, dy=111e3/4, dis_cal_sign = 'y', final_mask = None):
        self.lat_grid = lat_grid
        self.lon_grid = lon_grid
    ...
    def solve(self):
        ...
        sf_source = self.create_source_term(self.div)
        vf_source = self.create_source_term(self.vort)
        sf = self.solve_poisson_with_scaling(self.lat_grid, sf_source, self.dx, self.dy, scaling_factors)
        vp = self.solve_poisson_with_scaling(self.lat_grid, vf_source, self.dx, self.dy, scaling_factors)
