...
class WindFieldSolver:
    ...
    #######################################################################
    def __init__(self, lat_grid, lon_grid, center_lat, center_lon, vort, div, radius=333, dx=111e3/4, dy=111e3/4, dis_cal_sign = 'y', final_mask = None, tol=1e-6, max_iter = 1000):
        self.lat_grid = lat_grid
        self.lon_grid = lon_grid
    ...
    @staticmethod
    @njit(parallel=True)
    def solve_poisson_with_scaling(lat_grid, b, dx, dy, scaling_factors, tol=1e-6, max_iter=1000):
            ...
            for i in range(1, lat_grid.shape[0]-1):
                for j in range(1, lat_grid.shape[1]-1):
                    p[i, j] = ((p_old[i+1, j] + p_old[i-1, j]) * dy**2 +
                            ...
                            b[i, j] * dy**2 * dx**2 * scaling_factors[i, j]**2) / (
                                2 * (dy**2 + dx**2 *scaling_factors[i, j]**2)
                            )
            # 수렴 검사
    ...
    # @staticmethod
    # @jit(nopython=True)
    # def solve_poisson_4th_order(lat_grid, b, dx, dy, scaling_factors, tol=1e-6, max_iter=1000):
    #     p = np.zeros_like(lat_grid)
    #     for it in range(max_iter):
    #         p_old = p.copy()
    #         for i in range(2, lat_grid.shape[0]-2):
    #             for j in range(2, lat_grid.shape[1]-2):
    #                 dx2 = dx**2 * scaling_factors[i, j]**2
    #                 dy2 = dy**2
    #                 # 4차 차분: 양쪽 2칸까지 사용!
    #                 px = (-p_old[i+2, j] + 16*p_old[i+1, j] - 30*p_old[i, j] + 16*p_old[i-1, j] - p_old[i-2, j]) / (12*dx2)
    #                 py = (-p_old[i, j+2] + 16*p_old[i, j+1] - 30*p_old[i, j] + 16*p_old[i, j-1] - p_old[i, j-2]) / (12*dy2)
    #                 # Poisson: px + py = -b
    #                 p[i, j] = -(b[i, j] - (px + py))
    #         diff = np.max(np.abs(p - p_old))
    #         if diff < tol:
    ...
    def solve(self):
        ...
        sf_data, vp_data, final_mask = self.create_source_term()
        # print("sf_data type:", type(sf_data), "shape:", getattr(sf_data, 'shape', None))
        # print("vp_data type:", type(vp_data), "shape:", getattr(vp_data, 'shape', None))
        # print("final_mask type:", type(final_mask), "shape:", getattr(final_mask, 'shape', None))
        sf = self.solve_poisson_with_scaling(self.lat_grid, sf_data, self.dx, self.dy, scaling_factors, tol=self.tol, max_iter=self.max_iter)
        vp = self.solve_poisson_with_scaling(self.lat_grid, vp_data, self.dx, self.dy, scaling_factors, tol=self.tol, max_iter=self.max_iter)
        # sf = self.solve_poisson_4th_order(self.lat_grid, sf_data, self.dx, self.dy, scaling_factors, tol=self.tol, max_iter=self.max_iter)
        # vp = self.solve_poisson_4th_order(self.lat_grid, vp_data, self.dx, self.dy, scaling_factors, tol=self.tol, max_iter=self.max_iter)
...
def find_large_groups(mid_pos, ens_num_list, tar_pos, nearby_sign, distance_threshold, rank, min_size):
    ...
    if len(sorted_groups) >= rank:
        ...
        group_idx_from_all = np.array([member for member in ens_num_list if member in target_group])
        # print(group_indices)
        group_tar_pos = tar_pos[group_indices]
        group_mid_pos = mid_pos[group_indices]
        
        return group_tar_pos, group_mid_pos, group_indices, group_idx_from_all
...
def weighted_sum(
    ...
    """
    Trapezoidal weighted sum along given axis.
    - data: input array (e.g., [pressure, lat, lon])
    - weight_list: array of pressure levels, same length as data.shape[axis]
    - axis: axis along which to apply weighted sum (default=0)
    - normalize: if True, normalize weights so that sum=1 (weighted mean)
    """
    data = np.asarray(data)
    weight_list = np.asarray(weight_list)
    n = data.shape[axis]

    # Trapezoidal weights (like np.trapz)
    delta = np.diff(weight_list)
    weights = np.zeros(n)
    ...
    # Shape alignment for broadcasting
    # e.g., if axis=0 and data shape is (4,20,30), weights shape -> (4,1,1)
    shape = [1] * data.ndim
    shape[axis] = n
