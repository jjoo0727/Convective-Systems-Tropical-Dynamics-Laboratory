...
axis_opt = 'quive'                             #axis 뭘로 잡을지, opt: 위치 상관관계 최대인 axis, tar: 최종 위치의 axis, mid: 중간 위치의 axis, lon: 경도, lat: 위도
...
for predict_interval in np.arange(66,total_time_range+1,6):
...
# for predict_interval in np.arange(120,124,6):
    ...
    tar_pos_all = np.copy(tar_pos)
    mid_pos_all = np.copy(mid_pos)
    ...
    for choosen_factor in choosen_factor_list:
        ...
        if choosen_factor == 'u' or choosen_factor == 'v':
            ...
            for uv_uni in total_remove_uv:
                ...
                for mp, uv, ens_uv in zip(mid_pos_all, uv_uni, ens_num_list):
    ...
    if 'u' in choosen_factor_list and 'v' in choosen_factor_list:
        ens_12 = np.array(list(map(lambda y: ens_num_list.index(y), group1 + group2)))
        uv_data_12 = np.stack((uv_all_one['u'], uv_all_one['v']), axis=-1)[ens_12]
        uv_data = np.stack((uv_all_one['u'], uv_all_one['v']), axis=-1)
        pca_uv = PCA(n_components=1)
        pca_uv.fit_transform(uv_data_12)
        ...
        # plt.plot(uv_all_alt['u'], steer_pres,marker = 'x', label = 'u_mean', alpha = 1, color = 'blue')
        # plt.plot(uv_all_1['u'], steer_pres,marker = '^', label = 'u_group1', alpha = 0.5, color = 'blue')
        # plt.plot(uv_all_2['u'], steer_pres,marker = 'o', label = 'u_group2', alpha = 0.5, color = 'blue')
        # plt.plot(uv_all_alt['v'], steer_pres,marker = 'x', label = 'v_mean', alpha = 1, color = 'red')
        # plt.plot(uv_all_1['v'], steer_pres,marker = '^', label = 'v_group1', alpha = 0.5, color = 'red')
        # plt.plot(uv_all_2['v'], steer_pres,marker = 'o', label = 'v_group2', alpha = 0.5, color = 'red')
        ...
        plt.show()       
        fig_dir = f'/home1/jek/Pangu-Weather/plot/Sensitivity/{storm_name}_{start_str}_{target_str}_{axis_opt}{nearby_sign_name}/wind_alt/'
        if not os.path.exists(f'{fig_dir}'):
            os.makedirs(f'{fig_dir}')
        else:
            pass
        
        plt.savefig(f'{fig_dir}/{predict_interval}h.png',bbox_inches='tight')
